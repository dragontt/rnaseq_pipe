...
def build_alignment(samples, genome_index):
	...
	for i,row in samples.iterrows():
		...
		if paired:
			recipe += 'hisat2 -p 2 --rg-id='+ sample + ' -x '+ genome_index +' --dta --rna-strandness RF -1 '+ seqs[0] +' -2 '+ seqs[1] +' -S '+ bam_file +'; '
		else:
...
def build_expression_quantification(samples, reference_gtf, gene_list):
	...
	for i,row in samples.iterrows():
		...
		prereq = 'alignment/hisat2/'+ sample_id +'/aligned_reads_sorted.bam'
...
def main(argv):
	...
	sys.stdout.write('all: $(HISAT2_ALIGNMENTS)')
