...
def parse_args(argv):
	...
	parser.add_argument('-f', '--reference_genome', 
						help='Genome reference in fastq format.')
	# parser.add_argument('-g', '--gene_list',
	# 					help='Gene list.')
	parser.add_argument('--variant_filter_expr', 
	...
	parser.add_argument('--variant_filter_name', 
...
def build_alignment(samples, genome_index):
	...
	for i,row in samples.iterrows():
		...
		recipe += ' -1 '+ seqs[0] +' -2 '+ seqs[1] if paired else ' -U '+ seqs[0]
...
def build_expression_quantification(samples, ref_gtf, tool='htseq'):
	...
	for i,row in samples.iterrows():
		...
		if tool == 'htseq':
			...
			recipe = 'rm -rf '+ target_dir +'; mkdir '+ target_dir +'; '
			recipe += 'htseq-count -f bam -i gene_id -s no -t gene '+ prereq +' '+ ref_gtf +' > '+ output_file
...
def build_variant_calling(samples, ref_genome, xxx, filter_expr, filter_name, tool="mutect2"):
	built_dict = {}
	for i,row in samples.iterrows():
		sample = '_'.join([row['RUN'],row['SAMPLE']])
...
def main(argv):
	...
	# vc_dict = build_variant_calling(samples, parsed.reference_genome, xxx, filter_expr=parsed.variant_filter_expression, filter_name=parsed.variant_filter_name)
	...
	sys.stdout.write('all: $(ALIGNMENTS) $(EXPRESSIONS)')
